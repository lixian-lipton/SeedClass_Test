...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  char rstatus[80];
  int rlen = snprintf(rstatus, sizeof(rstatus), "%d/%d", E.file_position_y + 1, E.number_of_rows);
  ...
  switch (key) {
    case ARROW_LEFT:
      if (E.file_position_x != 0) {
        E.file_position_x--;
      } else if (E.file_position_y > 0) { // 移动到上一行行尾
        E.file_position_y--;
    ...
    case ARROW_RIGHT:
      if (row && E.file_position_x < row->size) {
        E.file_position_x++;
      } else if (row && E.file_position_x == row->size) { // 移动到下一行行首
        E.file_position_y++;
        E.file_position_x = 0;
      }
      break;
    ...
    case PAGE_DOWN:
      {
        if (c == PAGE_UP) {
            ...
            if (E.file_position_y > E.number_of_rows) E.file_position_y = E.number_of_rows - 1;
        }
